...
| D39 | `GmvCompareTrendPanel` 使用 `/api/analytics/gmv-compare` 或 compare.summary KPI | legacy-cleanup-audit | War-Room 用 `summaryKpi` + dashboard series only |
| D40 | `frontend/src/components/dashboard/gmvCompareFetch.ts` | 仅 analytics 历史 | `analytics/AnalyticsGmvCompareTrendPanel` |
| D41 | `[dashboard-filter]` 日志 | 已废弃 | 仅 `[dashboard-contract]` |
| D42 | 实时大屏 `trendFallback \|\| gmvCompare` KPI | 导致双数据源 | 已移除 |
| D43 | `normalizeGmvCompareResponse` 推断 totals 作大屏 KPI | 与 summary 漂移 | War-Room 用 `normalizeGmvCompareSeriesOnly` |
