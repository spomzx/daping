...
log_info "纳入: ${TO_BACKUP[*]}（仅代码与配置，不含运行时数据）"

# 发布前备份：只打包可部署代码，排除 storage/cache/SQLite/日志等运行时目录
TAR_EXCLUDES=(
  ...
  --exclude='backend/storage/backups'
  --exclude='backend/storage/.locks'
  --exclude='backend/data/*.db'
  --exclude='backend/data/*.db-shm'
  --exclude='backend/data/*.db-wal'
)

log_info "排除: node_modules dist logs .git backups *.log backend/storage backend/data/*.db*"
