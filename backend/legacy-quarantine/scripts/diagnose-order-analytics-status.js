...
async function main() {
  ...
  if (!pool) {
    console.error('[diagnose-order-analytics-status] �?MySQL');
    process.exit(2);
  }
  ...
  /** �?--date 日历日对齐（避免 valid �?custom �?trend �?CURDATE 导致误报�?*/
  const dayQuery = {
    timeRange: 'custom',
    startDate: startYmd,
    endDate: startYmd,
  ...
  /** 商品排行 orders �?SKU 维度去重之和，可�?valid 订单总数不等；GMV 子集�?�?valid_gmv */
