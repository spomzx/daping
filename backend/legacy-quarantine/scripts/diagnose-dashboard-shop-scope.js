...
/** 与登�?JWT 一致：users + user_tenants，角色来�?ut.role */
...
async function main() {
  ...
  if (!pool) {
    console.error('[diagnose-dashboard-scope] �?MySQL');
    process.exit(2);
  }
  ...
  if (!Number.isFinite(tenantId) || tenantId <= 0) {
    console.error('[diagnose-dashboard-scope] 需要有�?tenant_id�?-tenant-id 或用户成员关系）');
    process.exit(1);
  }
  ...
  const report = {
    ...
    notes: [
      ...
      '大屏排行现已合并全部 eligible 店铺（无单显�?0），不再仅显示有订单的店',
