...
/**
 * 各模�?SQL WHERE 均经 buildDashboardWhere + contractForDashboardApi
...
async function main() {
  ...
  if (!pool) {
    console.error('[diagnose-dashboard-filter-contract] �?MySQL');
    process.exit(2);
  }
  ...
  const report = {
    ...
    notes: [
      ...
      'paid �?valid �?SQL 层分离（normalizeOrderFilter 保留 paid�?,
