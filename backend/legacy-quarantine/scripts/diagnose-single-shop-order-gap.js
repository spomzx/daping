...
/**
 * 单店订单差异诊断（只读）：平台后台单�?vs 大屏 valid 口径 vs API vs MySQL
 *
 ...
 * 不写库、不清订单、不�?KPI contract / worker / deploy�? */
...
function buildPossibleReason(ctx) {
  ...
  if (excluded > 0) {
    details.push({
      ...
      detail: `MySQL 事件窗内全量 ${ctx.mysql_all_orders_count}，valid ${ctx.mysql_valid_orders_count}；非 valid �?${excluded}（cancelled/unpaid/sample/other）`,
    });
  }
  ...
  if (
    ...
    details.push({
      ...
      detail: `平台后台�?${platform} 单接�?MySQL 全量 ${ctx.mysql_all_orders_count}，大�?valid ${ctx.dashboard_valid_count} �?多为「全部订单」vs「有效订单」口径差`,
    });
  }
  ...
  if (ctx.dashboard_valid_count === ctx.mysql_valid_orders_count && excluded > 0) {
    details.push({
      ...
      detail: '大屏 valid �?MySQL analytics_status=valid 一致；与平台全量差异来自状态筛�?,
    });
  }
  ...
  if (ctx.api_vs_mysql_missing_count > 0) {
    details.push({
      ...
      detail: `API 窗口�?${ctx.api_vs_mysql_missing_count} �?platform_order_id �?MySQL �?shop_id 下不存在（入库漏单或店铺映射错误）`,
    });
  }
  ...
  if (ctx.api_total_orders_count > ctx.mysql_all_orders_count + 3) {
    details.push({
      ...
      detail: `API ${ctx.api_total_orders_count} > MySQL 事件窗全�?${ctx.mysql_all_orders_count}，检查同�?worker / 分页上限 / 时区`,
    });
  }
  ...
  if (ctx.mysql_outside_date_count > 0) {
    details.push({
      ...
      detail: `${ctx.mysql_outside_date_count} �?create_at_platform �?API 日窗内，�?paid_at→created_at_platform→created_at 落在大屏 custom 日窗外`,
    });
  }
  ...
  if (ctx.api_pagination_cap_hit) {
    details.push({
      ...
      detail: '已拉�?20 页且末页仍有 next_page_token，API 结果可能被截断（最多约 2000 �?日）',
    });
  }
  ...
  if (ctx.dashboard_valid_count !== ctx.ranking_count || ctx.dashboard_valid_count !== ctx.summary_count) {
    details.push({
      ...
      detail: `valid=${ctx.dashboard_valid_count} ranking=${ctx.ranking_count} summary=${ctx.summary_count}，应一致（�?locked WHERE）`,
    });
  }
  ...
  if (
    ...
    details.push({
      ...
      detail: `--date=${ctx.date_ymd} 的大�?custom 窗与服务器「today」窗 epoch 不同；若大屏未切 custom 日，live today 可能不同`,
    });
  }
...
async function runShopDiagnosis(pool, tenantId, shopRow, dateYmd, platformOrders) {
  ...
  return {
    ...
    dashboard_metrics: {
      ...
      note:
        'dashboard_* 使用 --date �?custom 日窗 + locked valid；dashboard_live_today_valid_count 为服务器本地「today」窗（可能与 --date 不同�?,
    },
...
async function main() {
  ...
  if (!pool) {
    console.error('[diagnose-single-shop-order-gap] �?MySQL 连接');
    process.exit(2);
  }
