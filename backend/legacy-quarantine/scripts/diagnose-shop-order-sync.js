...
async function main() {
  ...
  if (!pool) {
    console.error('[diagnose-shop-order-sync] �?MySQL');
    process.exit(2);
  }
  ...
  if (!shop) {
    console.error('[diagnose-shop-order-sync] 未找到店�?);
    process.exit(1);
  }
  ...
  const report = {
    ...
    sync_eligibility: {
      ...
      worker_skip_note:
        '�?readSyncShops/worker 一致：不因 scope 字段缺失预判跳过；API 权限失败�?scope_error',
    },
    ...
    tiktok_order_api: {
      ...
      status_filter_used: apiRet.status_filter_used || 'none（未�?order_status�?,
    ...
    timezone_audit: {
      ...
      note:
        'OpenAPI 按市�?offset 自然日拉 create_time；大�?today 默认服务器本地日；MySQL 统计�?paid_at→created_at_platform→created_at',
    },
