...
async function refreshTenantShopHealth(pool, tenantId, _storageDir) {
  const [shopRows] = await pool.query(
    `SELECT id, tenant_id, platform, platform_shop_id, shop_name, display_name, market, region, currency,
            ...
            last_health_status, last_health_message, last_health_checked_at, created_at, updated_at
  ...
  const { shops: enriched, statsSource } = await enrichShopRows(pool, tenantId, shops);

  let abnormal = 0;
  for (const row of enriched) {
    if (isAbnormalHealthStatus(row.health_status)) abnormal += 1;
    await pool.execute(
      `UPDATE shops SET
         last_order_seen_at = ?,
         last_order_count = ?,
      ...
      [
        row.latest_order_at || row.last_order_seen_at,
        Number(row.today_orders) || 0,
        Number(row.today_gmv) || 0,
        row.latest_sync_success_at || row.last_sync_at,
        String(row.health_status || row.last_health_status).slice(0, 32),
        row.health_reason || row.last_health_message,
        row.id,
        tenantId,
      ],
    );
  ...
  return {
    checked: enriched.length,
    abnormal,
    stats_source: statsSource || 'mysql',
  };
}
