  ...
  for (const norm of normalizedShops || []) {
    ...
    if (existing) {
      await pool.execute(
        ...
        [shop_name, shop_name, market, region, currency, nowSql, existing.id, tid],
      );
      updated_count += 1;
    ...
    const [ins] = await pool.execute(
      ...
      [tid, platform, platform_shop_id, shop_name, shop_name, market, region, currency, nowSql],
    );
...
async function upsertShopAuthToken(pool, { tenantId, shopId, platform, norm, nowSql }) {
  ...
  const rawPayload = {
    ...
    shop_cipher: norm.shopCipher,
    shop_id: norm.shopId,
    authorized_at: nowSql,
  };

  const [rows] = await pool.query(
    'SELECT id FROM shop_auth_tokens WHERE tenant_id = ? AND shop_id = ? ORDER BY id DESC LIMIT 1',
    [tenantId, shopId],
  );
