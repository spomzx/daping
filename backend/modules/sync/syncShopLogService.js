...
async function startShopSyncLog(shop) {
  ...
  try {
    ...
    console.error('[sync-shop-log] begin failed:', e?.message || e);
    return null;
  }
}
...
async function endShopSyncLog(logId, shop, patch) {
  ...
  try {
    ...
    console.error('[sync-shop-log] finish failed:', e?.message || e, {
      logId,
