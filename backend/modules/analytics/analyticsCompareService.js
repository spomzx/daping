...
function normalizeGmvCompareQuery(q) {
  ...
  const orderFilter = normalizeOrderFilter(src.orderFilter ?? src.status);
  return {
    ...src,
    shop_id: shopMerged,
    groupBy: groupMerged,
...
function buildEmptyPayload(hours, groupBy, orderFilter = 'all') {
  ...
  const zeroSeries = gb === 'hour' ? buildZeroHourBuckets(maxHourIdx ?? 23) : { today: [], yesterday: [] };
  return {
    today: zeroSeries.today,
    yesterday: zeroSeries.yesterday,
    summary: { todayTotal: 0, yesterdayTotal: 0, changePercent: null },
    gmv_currency: 'USD',
    meta: {
      hours,
