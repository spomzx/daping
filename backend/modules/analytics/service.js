...
const { normalizeOrderFilter, orderIsSampleOrder } = require('../../lib/orderFilter');
const { buildOrderFilterWhere, logDashboardFilterLine } = require('../dashboard/filterBuilder');
const { resolveDashboardDateRange } = require('../dashboard/dateRange');
const {
...
async function getTopProducts(tenantId, q, auth = null) {
  ...
  const aq = contractToAnalyticsQuery(contract);
  ...
  const af = await buildAnalyticsFilter(pool, tenantId, aq, fo);
  if (af.invalidShop) {
    const empty = [];
    cacheSet(ck, empty);
    return empty;
...
async function getShopTrend(tenantId, q, auth = null) {
  ...
  const st = contract.orderFilter;
  const ck = cacheKey('shop-trend', cacheScope(tenantId, auth), {
    market: contract.market,
    shopId: contract.shopId,
    timeRange: contract.timeRange,
    startDate: contract.startDate,
  ...
  const shopPart = await timedQuery('resolveShop-trend', () =>
    resolveShopClause(pool, tenantId, contract.shopId, 'o', { allTenants: fo.allTenantShops }),
  );
  if (contract.shopId !== 'all' && !shopPart) {
    cacheSet(ck, []);
    return [];
  }

  const mkt = marketClauseOrdersOnly('o', contract.market);
  const orderF = buildOrderFilterWhere(contract.orderFilter, 'o');
  ...
  const timeClause = `${evt} >= FROM_UNIXTIME(?) AND ${evt} <= FROM_UNIXTIME(?)`;
  const timeParams = [contract.startSec, contract.endSec];
  ...
  const params = fo.skipTenant
    ? [...timeParams, ...(shopPart ? shopPart.params : []), ...mkt.params, ...orderF.params]
    : [tenantId, tenantId, ...timeParams, ...(shopPart ? shopPart.params : []), ...mkt.params, ...orderF.params];
  ...
  logDashboardFilterLine('order-volume', aq, { rows: out.length, points: out.length });
  cacheSet(ck, out);
  return out;
}
...
/**
 * @param {number} tenantId
 * @param {{ timeRange?: string, range?: string, sort?: string, limit?: number }} q
...
async function getTopShops(tenantId, q, auth = null) {
  ...
  logDashboardFilterLine('ranking', aq, { rows: out.length });
  cacheSet(ck, out);
  return out;
}
...
/**
 * @param {number} tenantId
 * @param {{ market?: string, shopId?: string, shop_id?: string, limit?: number, orderFilter?: string }} q
 */
async function getRecentOrders(tenantId, q, auth = null) {
  ...
  const af = await buildAnalyticsFilter(pool, tenantId, aq, fo);
  if (af.invalidShop) {
    logDashboardFilterLine('orders', q, { rows: 0, invalidShop: 1 });
    cacheSet(ck, []);
    return [];
  }
  ...
  const out = rows.map((row) => {
    ...
    return {
      ...
      is_sample:
        String(row.analytics_status || '').trim().toLowerCase() === 'sample' ||
        orderIsSampleOrder({
