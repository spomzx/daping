...
router.use(authRequired, tenantScope, effectiveTenantScope, attachDataScope, requireFullAccess);
...
router.get('/product-ranking', requireRole('viewer', 'admin', 'super_admin'), (req, res) => {
  ctrl.productRanking(req, res);
});

router.get('/order-volume', requireRole('viewer', 'admin', 'super_admin'), (req, res) => {
  ctrl.orderVolume(req, res);
});

/** MySQL-only 别名：订单走势 */
router.get('/order-trend', requireRole('viewer', 'admin', 'super_admin'), (req, res) => {
  ctrl.orderVolume(req, res);
});
...
/** MySQL-only 别名：GMV 趋势 */
router.get('/gmv-trend', requireRole('viewer', 'admin', 'super_admin'), (req, res) => {
  ctrl.gmvCompare(req, res);
});
