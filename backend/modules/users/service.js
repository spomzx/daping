...
const {
  assertCanDeleteUser: assertCqchicPrimaryAdminDelete,
  softDeleteTenantIfNoActiveAdmin,
} = require('./tenantAdminSync');
const {
  assertTenantActive,
  assertUserLimit,
  PlanGuardError,
...
/**
 * 登录诊断：不过滤 tenant/membership 活跃状态（用于精确错误码）。
 * @param {import('mysql2/promise').Pool} pool
 * @param {string} username
 */
async function findLoginDiagnosticByUsername(pool, username) {
  const u = String(username || '').trim();
  if (!u) return null;
  const [rows] = await pool.query(
    `SELECT
...
async function rejectPendingTenantOwner(pool, auth, targetUserId) {
  ...
  const { isProtectedTenant, logBlockedProtectedTenant } = require('../../lib/tenantSafety');
  if (trow && isProtectedTenant(trow)) {
    logBlockedProtectedTenant(trow, {
      action: 'disable_tenant_blocked',
      reason: 'reject_pending_owner',
      operator: `user:${auth.user_id}`,
    });
    return { ok: false, status: 403, error: 'protected_tenant' };
  }
  const { insertNotification } = require('../notifications/service');
  const conn = await pool.getConnection();
  try {
...
async function deleteUserByActor(pool, auth, targetUserId) {
  ...
  const cqBlock = await assertCqchicPrimaryAdminDelete(pool, uid, memberships);
  if (cqBlock) return cqBlock;

  const adminTenantIds = [
    ...new Set(
      memberships
        .filter((m) => isAdminFamily(m.role))
  ...
  const conn = await pool.getConnection();
  /** @type {{ id: number, tenant_name: string, tenant_code: string }[]} */
  const tenantsDeleted = [];
  try {
    await conn.beginTransaction();
    await conn.execute('DELETE FROM user_shop_permissions WHERE user_id = ?', [uid]);
    ...
    for (const tid of adminTenantIds) {
      const upd = await softDeleteTenantIfNoActiveAdmin(conn, tid);
      if (upd.warning) {
        tenantsDeleted.push({
          id: upd.tenantId,
          tenant_name: upd.tenant_name,
          tenant_code: upd.tenant_code,
          status: 'warning',
        });
      }
    }
    ...
    return { ok: true, tenants_deleted: tenantsDeleted };
  } catch (e) {
    await conn.rollback();
    throw e;
  } finally {
