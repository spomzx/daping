...
function isTenantPlanWritable(row) {
  ...
  if (ts === 'disabled' || ts === 'deleted') return false;
  return true;
}

const TENANT_PLAN_SELECT = `id, tenant_code, tenant_name, status, plan_type, shop_limit, max_users, max_shops,
  expires_at, is_active, plan_remark, current_users, updated_by, base_currency, timezone, created_at, updated_at`;
...
async function listTenantsPaged(pool, query) {
  ...
  if (planStatus === 'expired') {
    ...
    where.push('is_active = 1 AND (expires_at IS NULL OR expires_at > DATE_ADD(NOW(), INTERVAL 7 DAY))');
  }
