...
async function fetchTodayOrders(shop, opts = {}) {
  ...
  const region = String(shop?.region || '').trim().toUpperCase();
  const range = region === 'TH' || region.includes('THAILAND') ? getTodayRangeByOffsetHours(7) : getTodayRangeByOffsetHours(8);
  ...
  for (let i = 0; i < maxPages; i += 1) {
      ...
      JSON.stringify(
        {
          ...
          finalUrl: ret?.debug?.finalUrl || ret?.debug?.orderUrl || '',
          page: i + 1,
          page_size: pageSize,
    ...
    if (!ret.ok) {
      ...
      return { ...ret, debug: ret.debug, requestPages, pagesFetched: requestPages.length, pageSize };
    }
