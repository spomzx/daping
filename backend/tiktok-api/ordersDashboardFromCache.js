...
function buildOrdersDashboardPayload(rawOrders, opts = {}) {
  ...
  /** MySQL 已走 buildDashboardWhere；禁止 createTime 二次截断（会导致与 summary 双口径） */
  const mysqlContractFiltered = opts.mysqlContractFiltered === true;
  let timeScoped = list;
  if (!mysqlContractFiltered) {
    const { startSec, endSec } = bounds;
    timeScoped = list.filter((o) => {
      const ep = getCreateEpochSec(o);
      if (!ep) return false;
      return ep >= startSec && ep <= endSec;
    });
    if (process.env.DASHBOARD_LEGACY_TIME_FILTER_LOG === '1') {
      console.warn('[dashboard-time-filter][deprecated]', {
        range: rangeNorm,
        startDate: bounds.startDate,
        endDate: bounds.endDate,
        before: list.length,
        after: timeScoped.length,
      });
    }
  }

  const filteredOrders = filterOrdersByMarket(timeScoped, marketFilter);
