...
const { getConversionRate, getFallbackRate, safeAmount } = require('../lib/rates');
...
async function convertAmount(value, fromCurrency, toCurrency) {
  ...
  const rate = conv.rate > 0 ? conv.rate : getFallbackRate(from, to);
...
async function buildGmvPayload({ allOrders, baseCurrency, targetCurrency }) {
  ...
  return {
    ...
    baseCurrency,
    targetCurrency,
    exchangeRate: (await getConversionRate(baseCurrency, targetCurrency, false)).rate || getFallbackRate(baseCurrency, targetCurrency),
    summary: {
