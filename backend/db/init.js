...
const { migrateSyncStability41 } = require('./migrateSyncStability41');
const { migrateDashboardPerf43 } = require('./migrateDashboardPerf43');
const { migrateDashboardRollup44 } = require('./migrateDashboardRollup44');
const { migrateTenantsDeletedAt42 } = require('./migrateTenantsDeletedAt42');
...
async function runMysqlMigrateAndSeed() {
  ...
  try {
    ...
    await migrateSyncStability41(connection);
    await migrateDashboardPerf43(connection);
    console.log('[mysql] migrateDashboardPerf43 ok');
    const rollup44 = await migrateDashboardRollup44(connection);
    console.log('[mysql] migrateDashboardRollup44 ok', rollup44);
    const migrate42 = await migrateTenantsDeletedAt42(connection);
    console.log('[mysql] migrateTenantsDeletedAt42', migrate42);
    const { verifySyncStabilitySchema } = require('./migrateSyncStability41');
    const syncSchema = await verifySyncStabilitySchema(connection);
    console.log('[mysql] migrateSyncStability41 ok', syncSchema.ok ? 'verified' : syncSchema.missing);
