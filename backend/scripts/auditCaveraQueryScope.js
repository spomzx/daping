...
/**
 * Cavera 查询口径核查（不改同步逻辑）
 * 用法：cd backend && node scripts/auditCaveraQueryScope.js [platform_order_id]
 */
...
function classify(ctx) {
  ...
  if (ctx.wrong_query_used_platform_id_on_orders_shop_id) {
    reasons.push('人工 SQL 将 orders.shop_id 与 platform_shop_id 比较（orders.shop_id 应为 shops.id）');
    code = 'query_wrong';
  }

  if (ctx.cavera_all_time > 0 && ctx.cavera_24h_by_correct_join === 0) {
    reasons.push(`Cavera 全量 ${ctx.cavera_all_time} 单，但滚动 24h 为 0；最新单 ${ctx.cavera_latest || 'null'}`);
    if (ctx.cavera_today_natural > 0 || ctx.cavera_today_platform_field > 0) {
      code = 'time_window_mismatch';
      reasons.push('自然日/created_at_platform 今日有单，与滚动 24h 不一致');
      ...
      reasons.push('TH 24h 有单但均非 Cavera 正确 JOIN');
    }
  }
  ...
  if (ctx.th_24h_total > 0 && ctx.cavera_24h_by_correct_join === 0 && !ctx.wrong_query_hit_cavera) {
    if (ctx.th_24h_only_cq_chic) {
      ...
      reasons.push('TH 24h 订单仅落在 CQ Chic 相关 shop_name/shop_id');
    }
  }

  if (ctx.cavera_all_time === 0 && ctx.wrong_query_hit_cavera > 0) {
    code = 'query_wrong';
    reasons.push('错误口径 shop_id=platform_shop_id 曾命中行，正确 JOIN 无单');
  }

  if (ctx.cavera_all_time === 0 && ctx.th_24h_total > 0) {
    if (code === 'unknown') code = 'mysql_no_orders';
    reasons.push('MySQL 无 Cavera 历史单；TH 今日单在他店');
  }
...
async function main() {
  ...
  if (!pool) {
    console.error('MySQL unavailable — 请在 staging 执行本脚本');
    process.exit(1);
  }
  ...
  const thDist = await q(
    pool,
    '3. TH 24h 按 orders.shop_id / shop_name 分布',
    `SELECT
       o.shop_id,
       o.shop_name,
       o.market,
  ...
  const caveraBangkokDay = await q(
    pool,
    '5d. Cavera — 曼谷自然日（与 OpenAPI TH 一致）',
    `SELECT COUNT(*) AS cnt, SUM(o.total_amount) AS gmv, MAX(${EVT}) AS latest_order_at
     FROM orders o
     INNER JOIN shops s ON s.id = o.shop_id
     WHERE LOWER(TRIM(s.platform_shop_id)) = ?
  ...
  if (tiktokOrderId) {
    ...
    console.log('\n=== 4. platform_order_id ===');
    console.log('未传入订单号；用法: node scripts/auditCaveraQueryScope.js <TikTok订单号>');
  }
...
-- 勿用: shop_id = '${PLATFORM_ID}'  （那是 platform_shop_id，不是 orders.shop_id）
`);
