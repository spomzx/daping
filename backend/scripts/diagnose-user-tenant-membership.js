'use strict';
/**
 * 用户 ↔ 租户归属诊断（users / user_tenants / tenants / user_shop_permissions）
 * 用法：node backend/scripts/diagnose-user-tenant-membership.js
 */
...
async function main() {
  ...
  if (!pool) {
    console.error('MySQL 不可用，请配置 DB_* 环境变量');
    process.exit(1);
  }
  ...
  console.log('\n========== 4. 各租户 current_users（platform_list，与平台 /users 同口径）==========\n');
  const perTenant = [];
  let sum = 0;
  ...
  console.log(`各租户 count 之和: ${sum}（多租户用户会被重复加总）`);
