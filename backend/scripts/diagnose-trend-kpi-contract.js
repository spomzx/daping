...
/**
 * 趋势 KPI 与 today/ranking 一致性诊断（optional-safe，禁止 throw 中断）
 *
...
/**
 * @param {unknown} payload — trend 响应或 rows 数组
 */
...
/**
 * 诊断口径：仅 kpi_totals.gmv_usd（ROUND(SUM(raw),2)）；禁止 points reduce。
...
async function runDiagnosis(tenantId, dateYmd) {
  ...
  /** 排行租户 GMV 与 dashboard 同 rollup（单店展示可 round，汇总禁止 SUM(ROUND(shop))） */
