...
function saasHealthFlags() {
  return {
    ...
    dashboardMysqlOnly: isDashboardMysqlOnly(),
  };
}

/** 大屏与 OpenAPI worker 默认 MySQL-only；设 DASHBOARD_MYSQL_ONLY=0 才写 orders-cache.json */
function isDashboardMysqlOnly() {
  return String(process.env.DASHBOARD_MYSQL_ONLY || '1').trim() !== '0';
}
