...
const { scopeIncludesOrderInfoDeclared } = require('./openApiWorkerEligibility');
...
function getOpenApiShopsSource() {
  const src = String(process.env.OPENAPI_SHOPS_SOURCE || 'mysql').trim().toLowerCase();
  if (src === 'json') {
    console.warn(
      '[read-sync-shops] OPENAPI_SHOPS_SOURCE=json — legacy shops.json（禁止生产默认；仅紧急回滚）',
    );
  }
  return src;
}
...
async function readSyncShopsFromMysql(pool, opts = {}) {
  ...
  for (const r of Array.isArray(rows) ? rows : []) {
    ...
    const declared_scope_has_order_info = scopeIncludesOrderInfoDeclared(r.scope_json, r.raw_auth_json);
    ...
    eligible.push({
      ...
      rawTokenPayload,
      declared_scope_has_order_info,
      scope_has_order_info: declared_scope_has_order_info,
      enabled: true,
      status: 'active',
...
async function patchMysqlShopSyncState(internalShopId, patch = {}) {
  ...
  if (patch.lastSyncOk === true) {
    const hs =
      Number(patch.todayOrderCount) > 0
        ? 'normal'
        : String(patch.healthStatus || 'no_orders_today');
    fields.push('last_health_status = ?');
    params.push(hs);
    fields.push('last_health_message = NULL');
    fields.push('last_health_fail_count = 0');
  } else if (patch.lastSyncOk === false) {
    fields.push("last_health_status = 'warning'");
  }
...
async function loadOpenApiSyncStateFromMysql() {
  ...
  const [rows] = await pool.query(
    `SELECT s.id, s.platform_shop_id, s.last_sync_at, s.last_health_status, s.last_health_message, s.sync_enabled,
            ss.last_success_sync_at, ss.last_error, ss.sync_status
     FROM shops s
     LEFT JOIN shop_sync_status ss ON ss.shop_id = s.id AND ss.tenant_id = s.tenant_id AND ss.platform = s.platform
     WHERE s.platform = 'tiktok' AND s.status <> 'deleted'`,
  );

  for (const r of Array.isArray(rows) ? rows : []) {
    const pid = String(r.platform_shop_id || '').trim().toLowerCase();
    if (!pid) continue;
    const successMs = r.last_success_sync_at
      ? dayjs(r.last_success_sync_at).valueOf()
      : r.last_sync_at
        ? dayjs(r.last_sync_at).valueOf()
        : 0;
    const syncSt = String(r.sync_status || '').toLowerCase();
    const recentSuccess =
      successMs > 0 &&
      (syncSt === 'success' ||
        syncSt === 'idle' ||
        syncSt === 'partial_success' ||
        ['normal', 'no_orders_today', 'ok'].includes(String(r.last_health_status || '').toLowerCase()));
    map.set(pid, {
      lastSyncAtMs: successMs || (r.last_sync_at ? dayjs(r.last_sync_at).valueOf() : 0),
      lastSyncOk: recentSuccess,
      lastSyncError: recentSuccess ? '' : String(r.last_error || r.last_health_message || ''),
