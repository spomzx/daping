...
import { OperationLogCenterPage } from './pages/Logs/OperationLogCenterPage'
import { SyncCenterPage } from './pages/SyncCenter/SyncCenterPage'
import { SyncJobsPage } from './pages/SyncJobs/SyncJobsPage'
...
function AppShell() {
  ...
  if (loc.pathname === '/admin/sync-jobs') {
    if (isViewerLike(session.role) || !isAdminLike(session.role)) {
      return <Navigate to={SAAS_CONSOLE_HOME} replace />
    }
    return withPlatformView(saasWrap('saas.nav.syncJobs', <SyncJobsPage appRole={session.role} />))
  }
  ...
  if (loc.pathname === '/logs') {
    ...
    return saasWrap(
      'saas.nav.logs',
      <OperationLogCenterPage appRole={session.role} appUserScope={session.scope} />,
    )
  }
