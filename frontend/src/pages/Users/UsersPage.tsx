    ...
    <div className="saas-admin-page users-page">
      ...
      >
        {platformSession && pendingRows.length > 0 ? (
          <div className="users-pending-box">
            <h3>{t('users.pendingTitle')}</h3>
                  ...
                  {pendingRows.map((u) => {
                    ...
                    return (
                      <tr key={`p-${id}`}>
                        ...
                        <td>
                          <button type="button" disabled={busy} className="refresh-btn" onClick={() => setViewRow(u)}>
                            {t('btn.view')}
                          </button>
                          <button
                            type="button"
        ...
        <div className="saas-table-wrap">
          <table className="saas-table">
            <thead>
              <tr>
                <th>{t('users.thUser')}</th>
                <th>{t('users.thContact')}</th>
                <th>{t('users.thRole')}</th>
                <th>{t('users.thStatus')}</th>
                <th>{t('users.thCustomer')}</th>
                <th>{t('users.thAssignedShops')}</th>
                <th>{t('users.thCreatedAt')}</th>
                <th>{t('users.thActions')}</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((u, i) => {
                ...
                return (
                  <tr key={`${u.id ?? u.username}-${u.tenant_id ?? i}`}>
                    <td>
                      <SaasCellStack
                        title={resolveUserNickname(u)}
                        subtext={resolveUserLoginAccount(u)}
                    ...
                    <td>{u.contact?.trim() || '—'}</td>
                    <td>{displayRole(u.scope, u.role)}</td>
                    <td>{statusText}</td>
                    <td>
                      <SaasCellStack
                        title={resolveTenantTitle(u)}
                    ...
                    {!platformSession ? <td className="num">{u.assigned_shop_count ?? 0}</td> : null}
                    <td>{formatDt(u.created_at)}</td>
                    <td className="users-actions-cell">
                      <div className="users-actions-inner">
                        {showAssign ? (
                          <button type="button" className="refresh-btn" onClick={() => setAssignTarget(u)}>
                            {t('users.assignShops')}
                          </button>
                        ) : null}
                        {isSelf ? (
                          <button
                            ...
                            className="refresh-btn"
                            disabled
                            title={t('users.changePasswordSoon')}
                          >
                          ...
                          <>
                            {showAct ? (
                              <>
                                ...
                                <button type="button" disabled={busy} className="refresh-btn" onClick={() => openReset(u)}>
                                  {t('btn.resetPassword')}
                                </button>
                              </>
                            ) : null}
                            {showDelete ? (
                              <button type="button" disabled={busy} className="refresh-btn" onClick={() => void deleteViewer(u)}>
                                {t('btn.delete')}
                              </button>
                            ) : deleteHint ? (
        ...
        <div className="users-modal-backdrop" onClick={() => setViewRow(null)}>
          <div className="users-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="users-modal__actions">
              <button type="button" className="refresh-btn" onClick={() => setViewRow(null)}>
                {t('btn.close')}
              </button>
            </div>
          </div>
        ...
        <div className="users-modal-backdrop" onClick={() => !cBusy && setCreateOpen(false)}>
          <div className="users-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="users-modal__actions">
              ...
              <button type="button" disabled={cBusy} className="refresh-btn" onClick={() => setCreateOpen(false)}>
                {t('btn.cancel')}
              </button>
            </div>
          </div>
      ...
      {resetOpen && resetTarget ? (
        <div className="users-modal-backdrop" onClick={() => !rBusy && setResetOpen(false)}>
          <div className="users-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="users-modal__actions">
              <button type="button" disabled={rBusy} className="refresh-btn" onClick={() => void submitReset()}>
                {rBusy ? t('btn.submitting') : t('users.confirmReset')}
              </button>
              <button type="button" disabled={rBusy} className="refresh-btn" onClick={() => setResetOpen(false)}>
                {t('btn.cancel')}
              </button>
            </div>
          </div>
